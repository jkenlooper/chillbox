...
test -n "$PUBLIC_SSH_KEY_LOCATIONS" || (echo "ERROR $script_name: PUBLIC_SSH_KEY_LOCATIONS variable is empty. Is it set in the $env_config file?" && exit 1)
test -n "$PUBLIC_SSH_KEY_FINGERPRINT_ACCEPT_LIST" || (echo "ERROR $script_name: PUBLIC_SSH_KEY_FINGERPRINT_ACCEPT_LIST variable is empty. Is it set in the $env_config file?" && exit 1)
...
echo "INFO $script_name: Processing environment variable PUBLIC_SSH_KEY_LOCATIONS"
echo "$PUBLIC_SSH_KEY_LOCATIONS"
echo "INFO $script_name: Processing environment variable PUBLIC_SSH_KEY_FINGERPRINT_ACCEPT_LIST"
echo "$PUBLIC_SSH_KEY_FINGERPRINT_ACCEPT_LIST"
echo "INFO $script_name: Creating $ssh_keys_file"
...
for ssh_key_location in ${PUBLIC_SSH_KEY_LOCATIONS}; do
  ...
  if [ "$ssh_key_location" != "${ssh_key_location#'https://'}" ]; then
    ...
    if [ "$ssh_key_location" = "${ssh_key_location#'/'}" ]; then
      echo "ERROR $script_name: The public ssh key location found is not an absolute file path: $ssh_key_location"
      exit 1
    fi
    if [ ! -e "$ssh_key_location" ]; then
      echo "ERROR $script_name: The public ssh key file ($ssh_key_location) does not exist."
      exit 1
    fi
...
jq --exit-status '.developer_public_ssh_keys[]' "$ssh_keys_file" > /dev/null || (echo "ERROR $script_name: No public ssh keys set in $ssh_keys_file." && exit 1)
