...
service_secrets_config_exists="$(aws \
  ...
  s3 ls \
  "s3://$ARTIFACT_BUCKET_NAME/chillbox/encrypted-secrets/$service_secrets_config_dir/$CHILLBOX_GPG_KEY_NAME/$service_secrets_config_file_name" > /dev/null \
    && printf 'yes' \
    || printf 'no')"
if [ "$service_secrets_config_exists" = "no" ]; then
  echo "INFO $0: Service secrets config doesn't exist at: s3://$ARTIFACT_BUCKET_NAME/chillbox/encrypted-secrets/$service_secrets_config_dir/$CHILLBOX_GPG_KEY_NAME/$service_secrets_config_file_name"
...
aws \
  --endpoint-url "$S3_ENDPOINT_URL" \
  s3 cp "s3://$ARTIFACT_BUCKET_NAME/chillbox/encrypted-secrets/$service_secrets_config_dir/$CHILLBOX_GPG_KEY_NAME/$service_secrets_config_file_name" \
...
echo "INFO $0: Decrypting file at s3://$ARTIFACT_BUCKET_NAME/chillbox/encrypted-secrets/$service_secrets_config_dir/$CHILLBOX_GPG_KEY_NAME/$service_secrets_config_file_name to $decrypted_file"
