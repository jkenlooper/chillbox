...
=== Add _alice_ as current user

. Run the `chillbox init` command again. Note that this time it will prompt for
the 'current_user' to be set. Enter _alice_ as the current user for this
...
----
<1> The current user is _alice_ for this tutorial.
...
This is because no 'public_ssh_key' was set for _alice_ in the chillbox
configuration file. The private ssh key is encrypted in the chillbox archive
directory.
+
A prompt to set a password for _alice_ will also happen at this point. Only
the hash of this password is saved in the chillbox archive directory. It can
be used when adding _alice_ to a server.

. Persist the password_hash and public_ssh_key that was created for _alice_.
...
=== Add _bob_ User That Has A Public SSH Key

. Add another user named _bob_ to the `chillbox.toml.` Bob has shared his public
...
since she is the current user and the 'owner' is set to _alice_. After
...
. Bob also has access to a secret that will need to be used on a server. Alice
   ...
   `chillbox.toml`. This time the 'owner' will be set to _bob_. This secret is
